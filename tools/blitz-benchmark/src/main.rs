...
fn main() {
    ...
    eprintln!("║  NOVACOL-545 — p95 SLA Gate + Regression Detection       ║");
    eprintln!("║  -- Ethos, AIML T1 Lead                                  ║");
    eprintln!("╚═══════════════════════════════════════════════════════════╝");
    eprintln!();
    ...
    // ── Publish aggregate to nova:alloy:benchmarks:latest (NOVACOL-638) ─────
    {
        let ts = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs();
        let alloy_payload = format!(
            r#"{{"novacol":"NOVACOL-638","agent":"alloy","timestamp":"{}","build":"{}","sla_gate":"{}","kernels_benchmarked":{},"batch_sizes":[1,8,32],"sla_failures":{},"regressions":{}}}"#,
            format_timestamp(ts),
            build_hash,
            sla_gate,
            all_results.len(),
        ...
        dragonfly(&["SET", ALLOY_LATEST_KEY, &alloy_payload, "EX", "86400"]);
        // Also update the tensor key for backwards compat
        dragonfly(&["SET", LATEST_KEY, &alloy_payload, "EX", "86400"]);
    ...
    eprintln!("-- ALLOY, DevOps T2 | NOVACOL-638");
    eprintln!("-- Ethos, AIML T1 Lead | NOVACOL-545");
...
fn generate_report(results: &[BenchResult], build_hash: &str) -> String {
    ...
    report.push_str("---\n\n");
    report.push_str("*Generated by blitz-benchmark v0.1.0 — Ethos, AIML T1 Lead*\n");
